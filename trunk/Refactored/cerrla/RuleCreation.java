...
public class RuleCreation implements Serializable {
	...
	public List<RelationalRule> rlggState(RelationalObservation observation)
			throws Exception {
		MultiMap<String, String[]> validActions = observation.getValidActions();
		Map<String, String> goalReplacements = observation.getGoalReplacements();
		// The relevant facts which contain the key term
		AgentObservations.getInstance().scanState(
				StateSpec.extractFacts(observation.getStateObservations()), goalReplacements);
