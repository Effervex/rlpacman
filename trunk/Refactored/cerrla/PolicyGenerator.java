...
public final class PolicyGenerator implements Serializable {
	...
	public List<RelationalRule> triggerRLGGCovering(
			RelationalObservation observation,
		...
		if (!frozen_
				&& AgentObservations.getInstance().isCoveringNeeded(
						observation.getValidActions(), activatedActions)) {
			List<RelationalRule> covered = null;
			try {
				covered = ruleCreation_.rlggState(observation);
	 ...
	 * @return The new PolicyGenerator.
	 */
	public static PolicyGenerator newInstance(GoalCondition goalCondition) {
		instance_ = new PolicyGenerator();
