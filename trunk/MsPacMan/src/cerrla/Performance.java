...
public class Performance {
	...
	public void saveFiles(PolicyGenerator policyGenerator,
		...
		if (policyGenerator.isModuleGenerator()) {
			...
			tempPerf = new File(LearningController.TEMP_FOLDER, Config
					.getInstance().getPerformanceFile().getName()
					+ runIndex_);
		}
	...
	private void saveElitePolicies(Collection<PolicyValue> elites)
			throws Exception {
		File outputFile = new File(LearningController.TEMP_FOLDER, Config
				.getInstance().getElitesFile().getName());
	...
	private void savePerformance(PolicyGenerator policyGenerator,
		...
		if (finalWrite) {
			...
			buf.write("Total run time: "
					+ RRLExperiment.toTimeFormat(System
