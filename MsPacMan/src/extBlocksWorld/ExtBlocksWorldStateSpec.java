...
public class ExtBlocksWorldStateSpec extends StateSpec {
	...
	protected Map<String, String> initialiseBackgroundKnowledge() {
		...
		// On(X,Y) -> Above(X,Y)
		bkMap.put("aboveRule1", "(on ?X ?Y) => (assert (above ?X ?Y))");

		// On(X,Y) & Above(Y,Z) -> Above(X,Z)
		bkMap.put("aboveRule2",
				"(on ?X ?Y) (above ?Y ?Z) => (assert (above ?X ?Z))");

		// Alphabetical precedence and vice-versa
		bkMap.put("precedence", "(block ?X) (block ?Y&:(< ?X ?Y)) "
				+ "=> (assert (precedes ?X ?Y))");
